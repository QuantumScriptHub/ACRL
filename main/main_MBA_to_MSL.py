...
def main(args):
    """
    主控脚本，用于启动 MBA 到 MSL 的领域自适应训练和评估任务。
    (版本：适配 'python -m' 模块化运行方式)
    """
    print(f"Starting experiment: Source=MBA -> Target={args.id_trg}")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Master script for MBA to MSL Domain Adaptation experiment")

    parser.add_argument('--path_src', type=str, default='./datasets/MBA', help='Path to Source dataset (MBA)')
    parser.add_argument('--path_trg', type=str, default='./datasets/MSL_SMAP', help='Path to Target dataset (MSL_SMAP)')
    parser.add_argument('--id_src', type=str, default='mba', help='ID of Source dataset (placeholder for MBA)')
    parser.add_argument('--id_trg', type=str, default='D-14', help='ID of Target MSL channel (e.g., D-14, T-1)')
    ...
    parser.add_argument('--experiment_folder', type=str, default='MBA_to_MSL_exp', help='Folder to save experiment results')
